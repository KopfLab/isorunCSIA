...
#' @param size the size of the dialog, default is small
#' @param ... parameters for fileSelectorInput
modalFileSelectorInput <- function(id, open_label = "Select file", dialog_label = open_label, size = "large", close_label = "Select", link_wrapper = identity, ...) {
  ns <- NS(id)
  modal_dlg <- bsModal(ns("modal_dialog"), dialog_label, ns("modal_link"), size = size,
...
#' @param number_recent how many recent items to show
