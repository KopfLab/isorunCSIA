    ...
    map_chr(
      ~
        if (is.null(.x) || is.na(.x)) {
          ...
          stop(glue::glue("unsupported value type: {class(.x)[1]}"), call. = FALSE)
        }) %>%
    glue::glue_collapse(", ")
}
...
df_to_insert_sql <- function(df, table) {
  glue::glue("INSERT INTO {table} ({glue::glue_collapse(names(df), sep = ', ')}) VALUES {df_to_sql(df)}")
}
...
run_sql <- function(sql, con) {
  ...
  tryCatch(
    ...
    error = function(e) {
      glue::glue("SQL statement failed ('{sql}') with message:\n{e$message}") %>% stop(call. = FALSE)
    })
  return(result)
}
...
run_insert_sql <- function(df, table, con, quiet) {
  ...
  if (!quiet) glue::glue("{result} new record(s) created.") %>% message()
  return(result)
}
