...
run_gui <- function(base_dir = ".", ...) {
  ...
  app_dir <- system.file("gui", package = "isorunCSIA")
  if (app_dir == "")
    stop("Could not find GUI directory. Try re-installing `isorunCSIA`.", call. = FALSE)

  isorunCSIA:::run_gui_dev(base_dir = base_dir, app_dir = app_dir, ...)
}
...
run_gui_dev <- function(base_dir = ".", app_dir = file.path("inst", "gui"), ...) {
  ...
  if (!file.exists(app_dir))
    stop("App directory does not exist, something might be wrong with the `isorunCSIA` installation: ", app_dir)
